...
test_that("MGC is Valid", {
  ...
  res <- unlist(parallel::mclapply(1:nsim, function(i) {
    ...
    set.seed(seed.idx[i])
    return(mgc.test(X, Y, nperm=50)$p.value < alpha)
...
test_that("MGC Works with Distance and Non-Distance Matrices for X and Y", {
  ...
  test.xy <- suppressWarnings(mgc.test(X, Y, nperm=20, no_cores=1))
  test.dx <- suppressWarnings(mgc.test(DX, Y, nperm=20, is.dist.X=TRUE, no_cores = 1))
  test.dy <- suppressWarnings(mgc.test(X, DY, nperm=20, is.dist.Y=TRUE, no_cores = 1))
  test.dxy <- suppressWarnings(mgc.test(DX, DY, nperm=20, is.dist.X=TRUE, is.dist.Y=TRUE, no_cores = 1))
