...
sim_leg <- g_legend(
  sim_twod_scatter(X, Y, title="", xlab="", ylab="") +
    scale_color_manual(name="Class", values=mcols) + theme(legend.position='bottom')
)

grid.arrange(arrangeGrob(grobs=plots, ncol=length(sims)), sim_leg, heights=c(0.9, 0.05))
```
