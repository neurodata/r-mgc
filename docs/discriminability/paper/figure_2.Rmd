...
sims.opts <- list(list(n=n, d=d, K=2, signal.lshift=0), list(n=n, d=d, K=2, signal.lshift=1),
...
sims.titles = c("(N) No Signal", "(A) 2 Class, Linear", #"(B) 5 Class, Linear", #"(C) 5 Class, Exponential",
                "(B) 2 Class, Cross", "(C) 2 Class, Spherical")#, "(F) 5 Class, Beta")
plots <- list()
...
dplots <- arrangeGrob(arrangeGrob(grobs=plots, ncol=length(sims)), sim_leg, widths=c(0.9, 0.05))
```
...
names(alg.colors) <- c("Discr", "ANOVA, best", "ANOVA, worst", "ICC, best", "ICC, worst", "MANOVA", "I2C2")
line.types <- c(1, 1, 2, 1, 2, 1, 1)
names(line.types) <- c("Discr", "ANOVA, best", "ANOVA, worst", "ICC, best", "ICC, worst", "MANOVA", "I2C2")
```

### Convergence of Discriminability

```{r}
discr.os.results <- subset(one.sample.results, alg=="Discr")
...
os.plots <- lapply(unique(one.sample.results$sim.name), function(simn) {
  sim.ss <- subset(one.sample.results, sim.name == simn)
  sim.ss$reject <- as.numeric(sim.ss$pval < al)
  power.dat <- aggregate(reject ~ n + alg, data=sim.ss, FUN=mean)
  ggplot(power.dat, aes(x=n, y=reject, color=alg, linetype=alg)) +
    geom_line(size=1.5) +
