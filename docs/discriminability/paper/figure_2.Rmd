...
sims.opts <- list(list(n=n, d=d, K=2, signal.lshift=0), list(n=n, d=d, K=2, signal.lshift=1),
                  ...
                  list(n=n, d=d, K=2, signal.scale=20), list(n=n, d=d, K=2))#,
...
two.sample.results <- two.sample.results[two.sample.results$alg %in% c("ANOVA, best", "ANOVA, worst", "MANOVA", "discr"),]
...
ts.plots <- lapply(unique(two.sample.results$sim.name), function(simn) {
  sim.ss <- subset(two.sample.results, sim.name == simn)
  sim.ss$reject <- as.numeric(sim.ss$pval < al)
  power.dat <- aggregate(reject ~ n + alg, data=sim.ss, FUN=mean)
  ggplot(power.dat, aes(x=n, y=reject, color=alg, linetype=alg)) +
    geom_line(size=1) +
...
ts.plots <- lapply(os.plots, function(plot) {
  plot +
    guides(color=FALSE, linetype=FALSE)
})
