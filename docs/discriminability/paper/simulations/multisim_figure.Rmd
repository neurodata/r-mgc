...
sample.test.results <- readRDS('../data/sims/discr_sims_os.rds')$os.results %>%
  ...
  dplyr::ungroup() %>%
  dplyr::mutate(
        stat.name=recode_factor(stat.name, "discr" = "Discr", "icc" = "PICC",
...
#                             "Rotated"="(vi) Rotated")
  ...
  dplyr::mutate(test.name=factor(test.name),
         ...
         stat.name=factor(stat.name, levels=c("I2C2", "PICC", "Discr", "1-Bayes"),
                          ordered=TRUE))


alg.colors <- c("#c70000", "#ffa500", "#6699cc", "#3CB371")
line.types <- c(1, 1, 1, 1)
names(alg.colors) <- names(line.types) <- c("Discr", "PICC", "I2C2", "1-Bayes")
```
...
bound.plot <- sample.test.results %>%
  dplyr::filter(test.name == "(C) Comparison to Error") %>%
  dplyr::group_by(n, d, stat.name, sim.name, test.name) %>%
