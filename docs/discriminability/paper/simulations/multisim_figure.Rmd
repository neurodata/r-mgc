...
sample.test.results <- readRDS('../data/sims/discr_sims_os.rds')$os.results %>%
  ...
  dplyr::filter(sim.name %in% c("No Signal", "Cross", "Gaussian", "Ball/Circle", "XOR")) %>%
  dplyr::group_by(n, d, stat.name, sim.name, test.name) %>%
  dplyr::mutate(sigma.wt=(sigma - min(sigma))/(max(sigma) - min(sigma))) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(
        stat.name=recode_factor(stat.name, "discr"="SimilRR", "icc"="PICC", "i2c2"="I2C2", "bayes"="1-Bayes"),
  ...
  dplyr::mutate(test.name=factor(test.name),
         ...
         stat.name=factor(stat.name, levels=c("SimilRR", "PICC", "I2C2", "MMD", "1-Bayes"),
                          ordered=TRUE))


alg.colors <- c("#c70000", "#ffa500", "#6699cc", "#740ac4", "#3CB371")
line.types <- c(1, 1, 1, 1, 1)
names(alg.colors) <- names(line.types) <- c("SimilRR", "PICC", "I2C2", "MMD", "1-Bayes")
```
