...
data <- lapply(c("(i) Reliable", "(ii) Offset", "(iii) Outlier"), function (sim) {
...
names(data) <- c("(i) Reliable", "(ii) Offset", "(iii) Outlier")
```
...
results <- lapply(names(data), function(sim) {
  ...
  dcorr <- ksamp.os(dat$X, dat$Y, is.dist=FALSE)
  data.frame(Statistic=c(icc, discr, fpi, dcorr), Simulation=sim,
             Algorithm=c("ICC/I2C2", "Discr.", "Finger.", "Kernel"))
})
...
col.1 <- do.call(rbind, data) %>%
  ...
  mutate(Simulation=factor(Simulation, 
                           levels=c("(i) Reliable", "(ii) Offset", "(iii) Outlier"), ordered=TRUE),
         Z=factor(Z)) %>%
...
names(alg.colors) <- c("Discr.", "ICC/I2C2", "Finger.", "Kernel")
...
col.2 <- Dmtx.results %>%
  ggplot(aes(x=Var1, y=Var2, fill=Distance)) +
    geom_tile() +
    geom_tile(data=Dmtx.ranked %>% arrange(as.numeric(Rank == RankPair)), aes(x=Var1, y=Var2, color=color), size=.5) +
...
sim.dat <- do.call(rbind, results) %>%
  mutate(Algorithm=factor(Algorithm, ordered=TRUE, levels=c("Discr.", "Finger.", "ICC/I2C2", "Kernel"))) %>%
  ungroup() %>%
  group_by(Algorithm) %>%
  mutate(Simulation=factor(Simulation, 
                           levels=c("(i) Reliable", "(ii) Offset", "(iii) Outlier"), ordered=TRUE)) %>%
