...
sim.no_signal <- function(n, d, sigma=1) {
  ...
  samp <- sim_gmm(mus=cbind(rep(0, d), rep(0,d)), Sigmas=sigma*abind(diag(d), diag(d), along=3), n)
...
sim.linear_sig <- function(n, d, sigma=2) {
  ...
  Sigma[1, 1] <- 2
  Sigma[-c(1), -c(1)] <- 1
  Sigma[1,1] <- sigma  # sample 2 has changing covariance in signal dimension
...
sim.crossed_sig <- function(n, d, sigma=1) {
  Sigma <- diag(d); Sigmas <- abind(Sigma, Sigma, along=3)
  # crossed signal
  Sigmas[2,2,2] <- Sigmas[1,1,1] <- 5; Sigmas[1,1,2] <- Sigmas[2,2,1] <- sigma
  # add correlation
  Sigmas[1,2,1] <- Sigmas[2,1,1] <- sqrt(5)/2; Sigmas[1,2,2] <- Sigmas[2,1,2] <- -sqrt(5)/2  # pipeline 2 is less discriminable than pipeline 1
...
# 2 classes
sim.multiclass_gaussian <- function(n, d, K=16, sigma=1) {
  S.k <- diag(d)*1
  ...
  Sigmas <- abind(lapply(1:K, function(k) S.k), along=3)*sigma
...
sim.multiclass_ann_disc <- function(n, d, K=16, sigma=0.1) {
  # centers
  K.cent <- K/2
  mu.class <- rep(0, d)
  S.class <- diag(d)*sqrt(K)
  ...
  X <- do.call(rbind, lapply(1:K.cent, function(k) {
    ...
    X[1:ni,] <- sweep(mgc.sims.2ball(ni, d, r=1, cov.scale=sigma), 2, mus[,k], "+")
    X[(ni + 1):(2*ni),] <- sweep(mgc.sims.2sphere(ni, r=1, d=d, cov.scale=sigma), 2, mus[,k], "+")
    return(X)
  }))

  Y <- sapply(1:K, function(k) rep(k, ni))
  return(list(X=X, Y=Y))
}
