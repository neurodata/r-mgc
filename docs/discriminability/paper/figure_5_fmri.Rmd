...
fmri.results$wt.discr <- fmri.results$discr*fmri.results$nsub
fmri.summarized <- aggregate(cbind(discr, nsub) ~ Name, data=fmri.results[1:200,], function(x) {
  #1/sum(x$nsub)*nan.mean(x$nsub*x$discr, na.rm=TRUE)
  print(x[1])
})

test <- data.frame(Name=c("A", "B", "A", "B"), discr=c(0.5, 0.3, 0.6, 0.25), nsub=c(1, 4, 1, 4))
```

```{r}
fmri.best.pipe <- as.character(fmri.summarized[which.max(fmri.summarized$me.discr),]$Name)
